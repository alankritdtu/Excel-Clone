...
$("#formula-input").on("blur", function () {
    ...
    if(!isValidFormula($(this).val() , cellAddress)){
        dialog.showMessageBox({ 
            ...
            icon: '', 
            title: 'Error', 
            message: 'There are one or more circular refrences where a formula refers to its own cell either directly or inderctly. This might cause them to calculate incorrectly.', 
            detail: 'Try removing or changing these refrences, or moving the formula to different cells.', 
...
function isValidFormula(formula , currCell){
    let {rowId,colId} = getRcFAddr(currCell) ;
    ...
    for (let i = 0; i < formulaComponent.length; i++) {
        ...
        if (code >= 65 && code <= 90) {
            let parent = getRcFAddr(formulaComponent[i]);
            if(parent.rowId==rowId && parent.colId==colId) return false ;
            else{
                if(!checkParents(parent , currCell)) return false ;
            }
        }
    }
    return true ;
...
function checkParents(parent,currCell){
    let {rowId,colId} = getRcFAddr(currCell) ;
    
    let parentObj = db[parent.rowId][parent.colId] ;
    let upstrmArr = parentObj.upstream ;

    for(let i=0;i<upstrmArr.length;i++){
        let ppObj = upstrmArr[i] ;
        if(ppObj.rowId==rowId && ppObj.colId==colId){
            return false ;
        }
        else{
            if(!checkParents(ppObj , currCell)) return false ;
        }
    }
